// DIRECT PASTE VERSION - Copy and paste this entire script into the service worker console
...
chrome.storage.sync.get(['blacklist', 'whitelist']).then(result => {
  ...
  if (result.blacklist && result.blacklist.length > 0 && (!blacklist || blacklist.length === 0)) {
    ...
    if (typeof init === 'function') {
      init().then(() => {
        ...
        console.log('- blacklist length after manual init:', blacklist ? blacklist.length : 'undefined');
        console.log('- whitelist length after manual init:', whitelist ? whitelist.length : 'undefined');
        ...
        if (blacklist && blacklist.length > 0) {
          ...
          if (typeof checkUrlShouldBeBlocked === 'function') {
            ...
            console.log('- checkUrlShouldBeBlocked function not available');
          }
      ...
      console.log('❌ CRITICAL: init() function is not defined!');
    }
